...
public class MetaOAuthClient : IMetaOAuthClient
{
    ...
    public Task SubscribePageWebhooksAsync(
        string platformCode,
        MetaOAuthCredentials credentials,
        MetaPageInfo page,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(page.PageAccessToken))
            throw new InvalidOperationException("no page access token was returned by Meta.");

        var version = NormalizeGraphVersion(credentials.GraphApiVersion);
        var code = (platformCode ?? string.Empty).Trim().ToLowerInvariant();
        var fields = code == "instagram"
            ? MetaGraphClient.InstagramPageSubscribedFields
            : MetaGraphClient.PageSubscribedFields;

        return _graph.SubscribePageAsync(version, page.PageId, page.PageAccessToken!, fields, cancellationToken);
    }

    public Task<IReadOnlyList<string>> GetSubscribedFieldsAsync(
        string platformCode,
        MetaOAuthCredentials credentials,
        string pageId,
    ...
    {
        var version = NormalizeGraphVersion(credentials.GraphApiVersion);
        return _graph.GetPageSubscribedFieldsAsync(version, pageId, pageAccessToken, cancellationToken);
    }
